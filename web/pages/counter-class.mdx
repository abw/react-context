...
Another benefit of this "traditional" object-oriented approach is that your
class can extend an existing base class that implements some common
functionality.  The `@abw/badger-context` library includes a `Context`
class for just that purpose.  It implements some of the boilerplate code
that I find useful when I'm writing a context model class.

...
generic type parameters.  We then wrap it using the `Generator` function and
...
returned will automatically be typed.
...
In this example we don't need to define any additional types as the `size`
property has a default `string` type which Typescript can infer automatically.

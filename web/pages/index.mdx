...
<div className="success alert border">
  ...
  The [Model](model) is new to version 3 and provides a simpler, cleaner way
  to do what the `Generator()` function did in version 2.  However, it only
  works with functional components.  You can continue to use the `Generator()`
  function in existing code, especially if you're using class based components.

...
import { ReactModelLink } from '@/site/Links'
...
<Hero
  ...
  underview={
    <>
      It is simple and small (around 1.4kB compressed).  It has no
...
<div className="success alert border">
  <h3>Version 3</h3>
  This documentation is for **version 3** of `@abw/react-context` which is now
  written in Typescript for added type safety goodness.

  There is one breaking change between the versions: The `Generator()` function
  ...
  The documentation for **version 2** is [here](version2/introduction).

  There's also a new kid in town: <ReactModelLink/> is a smaller, simpler
  and more Typescript friendly alternative to React Context.  However,
  it only works with functional components.  If you're starting out anew and
  planning to use functional components then that might be a good place to
  start.  Otherwise you can continue to use React Context in existing code,
  especially if you're using class based components.
</div>

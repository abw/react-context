...
This is new for version 3 and is the preferred way to define context models
for new code using functional components.

These examples are written in Typescript.  If you're using plain old
...
other utility classes we're using here (`flex`, `gap-2` and `mar-t-4`).
...
model and the additional `color` property to set the color of the buttons.
...
properties the `Counter` expects and the data that it returns both conform to
those types.
